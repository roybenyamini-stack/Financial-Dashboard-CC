...
This is the first governance artifact in the Goose Documentation System. It establishes Documentation Governance only. Current Direction and the RFC workflow are named in §10 as future components of the same system — not designed here. The Boot Protocol and the Decision Log are no longer among them: the Boot Protocol is designed in `GOOSE_CORE_BOUNDARY.md` and manifested as `GOOSE_BOOT.md` (see §3); the Decision Log is manifested as `DECISIONS.md` (Artifact 006 — see §3, §10).
...
| Layer | Documents |
|---|---|
| Goose General | `GOOSE_CONSTITUTION.md`, `GOOSE_CORE_BOUNDARY.md`, `GOOSE_DOCUMENTATION_GOVERNANCE.md` (this document), `GOOSE_BOOT.md`, `GOOSE_KNOWLEDGE_ARCHITECTURE.md` (Artifact 005 — see §12), `DECISIONS.md` (Artifact 006 — the Closed Knowledge log; see §10); `docs/foundation/templates/**` — the companion-template family for these Foundation and Knowledge Architecture artifacts, mapped once here rather than naming some individually and leaving others implicit: `GOOSE_BOOT_TEMPLATE.md`, `CAPABILITY_TEMPLATE.md`, `KNOWLEDGE_MODEL_TEMPLATE.md`, `KNOWLEDGE_OBJECT_TEMPLATE.md`, `REVIEW_TEMPLATE.md`, `DERIVED_VIEWS.md` |
...
| Roy Reality Lab | `GOOSE_EXPEDITION_1_ASSESSMENT.md` — an evidence-graded audit of this repository's real code and data. It is classified here, not as Goose Financial, because it records repository *truth* discovered through investigation, not product policy or domain rules. `GOOSE_EXPEDITION_2_PROVIDENT_FUND_CAPITAL_EXEMPT.md` — the same evidence-graded discipline applied to a legal-research question (Amendment 190's `capital_exempt` bucket): primary-source findings from Income Tax Circular 2/2013 and Form 159, with an explicit open-items backlog. Not a Knowledge Object; canonical data model and runtime implementation remain unapproved as of this entry. `GOOSE_EXPEDITION_3_PROVIDENT_FUND_CLASSIFICATION_IMPLEMENTATION.md` — a repository-understanding artifact tracing how the current codebase parses, maps, persists, and consumes provident-fund/Amendment 190 classification fields. Repository Understanding Only, pending Product Owner review. |
...
The following are approved future milestones of the Goose Documentation System, deferred rather than designed here — not open questions, but components already accepted as part of the system's direction. (`GOOSE_BOOT.md` was previously listed here; it is no longer deferred — it exists as Artifact 004, mapped in §3. `DECISIONS.md` was also previously listed here; it is no longer deferred — it exists as Artifact 006, mapped in §3, holding the Closed Knowledge mechanism: the distinction between Discovery and ratified Decisions, and how `GOOSE_BOOT.md` §3 now points to it.)
