...
### v170.9 – Optimized Persistence & Yoav 4.0

**1. תיקון עמוק — שמירת ספייק "רועי" (Deep Fix)**
- `_simSaveUserEvents()` עודכנה: לפני שמירה לוחצת ל-localStorage, בודקת אם יש כבר `events_timeline` מ-Excel ומאחדת אותם עם האירועים הנוכחיים בזיכרון
- מונע ממצב SIMULATOR (`ffsApplyToSimulator()`) למחוק את אירועי הפנסיה של רועי מה-localStorage
- גרסה: v170.9 בכותרת ובתגובת HTML

**2. יואב 4.0 — "High-Net-Worth Optimist"**
- **השקעות — 3 נכסים:**
  - תיק מנייתי: 1,250K ₪ (חדש)
  - קרן השתלמות מנהלים: 450K ₪ (קיים)
  - חסכון טכנולוגי: 750K ₪ (חדש)
- **סה"כ נכסים: ~6M ₪** (השקעות 2.45M + נדל"ן 2.2M + ביטוח מנהלים 1.4M)
- **קרן פנסיה:** קצבה צפויה עודכנה ל-18,500 ₪/חודש (מ-18,000)
- **פרישה מאוזנת:** הכנסה 28,500 ₪ | הוצאות 23,500 ₪ | עודף 5,000 ₪/חודש
- **גרף:** תחזית עולה גם בפרישה (הכנסה > הוצאות)



## שינויים אחרונים (30/04/2026 — v170.8)

### v170.8 – Professional Seniority Sync

**1. תיקון קריטי — ספייק "רועי" (Roy's Timeline Spike)**
- באג: `absoluteInternalReset()` כתב ל-localStorage אחרי שניקה את `SIM_USER_EVENTS` בזיכרון — מה שגרם ל-`_simRestoreUserEvents()` לשחזר מערך ריק
- תיקון: הסרת `localStorage.setItem(_SIM_EVENTS_LS_KEY, ...)` מתוך `absoluteInternalReset()`
- עכשיו: ניקוי רק בזיכרון; ה-localStorage נשמר ושחזור האירועים עובד כראוי

**2. יואב 3.0 — "הסניור המנוסה"**
- הכנסה: 32,000 ₪/חודש (עלייה מ-28K)
- הוצאות: 22,000 ₪/חודש (עלייה מ-20K)
- חיסכון חודשי: 10,000 ₪/חודש (עלייה מ-8K)
- קרן פנסיה: מנורה מבטחים, קצבה 11,500 ₪/חודש, הפרשה 18.5%
- ביטוח מנהלים חדש: מנורה מבטחים, הון צבור 1,400K ₪, מקדם 200, ביטוח חיים 1,400K

**3. Guest Overview — מבט על לסימולטור**
- `ovRenderCashflowChart()`: בסימולטור — בונה 12 עמודות סינתטיות (הכנסות/הוצאות) מה-FFS income phase
- `ovRenderInvestChart()`: בסימולטור — מציג קו גדילה של תיק ההשקעות FFS ב-4%/שנה על 13 חודשים
- `ovRenderPensionCards()`: בסימולטור — מציג הון צבור, קצבה חודשית, ו-% הפרשה מה-FFS



## שינויים אחרונים (30/04/2026 — v170.7)

### v170.7 – Smooth Demo & KPI Transparency

**1. יואב 2.0 — פרופיל דמו משופר**
- קרן פנסיה מקיפה נוספה: מגדל, קצבה צפויה 7,000 ₪/חודש, הפרשה 6.25%
- הוצאות פרישה עודכנו: 22K → 20,000 ₪/חודש
- הכנסה בפרישה: 7,000 ₪ (מקרן פנסיה)
- שלב הכנסה נוסף: 28,000 ₪/חודש מגיל 45 עד 67 (מציג עלייה ברורה בגרף)

**2. כפתורי ביטול — Visual Default Focus**
- שני מודלים (Yoav overwrite + blank-slate): כפתור "ביטול" קיבל `border:2px solid #3b82f6` + `box-shadow` כחול
- מסמן ויזואלית שזהו הכפתור הבטוח / ברירת המחדל

**3. KPI Transparency — אייקוני מידע**
- Simulator header: `ⓘ` ליד "הון חזוי" ו-"תזרים נטו"
- Overview header: `ⓘ` ליד "הון חזוי לגיל 67" ו-"רווח חודש אחרון"
- Tooltip: "מחושב לפי הון נוכחי + צבירה שוטפת בתשואות שהוגדרו (נדל"ן 2.5%, השקעות 4%) עד גיל הפרישה"

**4. RTL Arrow Restoration — שורת הוראות**
- שורת ההוראות שונתה ל-`direction:ltr` כדי שזרימת השלבים תהיה טבעית משמאל לימין
- חיצים שונו מ-`←` ל-`⮕` — פונים לכיוון הנכון: שלב 1 ⮕ שלב 2 ⮕ שלב 3
