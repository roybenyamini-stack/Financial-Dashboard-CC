...
The canonical statement of this engine's business rules — the tax-tier bucket structure, report-date tax formula, the blended sensitivity coefficient, YTD/future-projection updates, partial-withdrawal proportionality, linkage treatment, and the liquidity/tax-taxability boundary — now lives at `docs/knowledge/study_fund/SF_TAX_MODEL.md` (`SF-TAX-MODEL`) and its eight constituent Knowledge Objects, not in this table. The rows below remain the code-location index; where a row previously restated a business rule, it now points at the owning Rule ID instead.
...
| `_sfCalculateTax(product, withdrawalPct, globalConfig)` | `app.js:20344-20602` | The core, pure (no DOM access) tax function. Returns `null`/"low confidence" figures if `product.rawXml` is falsy (`app.js:20360-20371` — a manually-created Study Fund with no XML import). Otherwise re-derives `joinDate` and `taxSegments` from the raw XML if not already present (`app.js:20382-20400`), tracks a confidence score (`high`/`medium`/`low`) based on data completeness (`app.js:20402-20465`), and for `sugMutzar===4` (Study Fund) computes: `isExemptEligible = seniority≥6y OR age≥retirementAge`; **taxable-tier (tikrat=2) profit is taxed regardless of `isExemptEligible`; exempt-tier (tikrat=1) profit's tax is waived only when `isExemptEligible`** (`app.js:20481-20522`). This is the XML-only-fallback side of the vesting-exemption question now reframed by `SF-LIQUIDITY-TAX-SEPARATION` (`docs/knowledge/study_fund/SF_LIQUIDITY_TAX_SEPARATION.md`) — see that object for the current canonical statement rather than the discrepancy analysis previously here. |
...
| 3-phase formula | `app.js:20745-20832` | The report-date/YTD/future-projection formula, applied identically to YTD real profit and to future simulated profit. Canonical statement, including the `effectiveTaxCoeff` formula and a longitudinal comparison against a candidate alternative allocation model: `SF-REPORT-DATE-TAX`, `SF-TAX-SENSITIVITY-COEFFICIENT`, `SF-FUTURE-PROJECTION` (`docs/knowledge/study_fund/`). |
| YTD salary-deposit detection | `app.js:20760-20814` | Priority order: manual override (`_sfManualDeposits`) → XML scan of `PerutHafkadotMetchilatShana` elements dated after Jan 1 of the current year → auto-fill extrapolation (1–6 months gap) from the last known deposit amount. Canonical statement: `SF-YTD-UPDATE` (`docs/knowledge/study_fund/SF_YTD_UPDATE.md`). |
...
1. Which vesting-exemption model is legally correct for Israeli Study Funds: the "full exemption of all layers after 6 years / retirement age" stated in `docs/TaxLogic.md` and `israel_tax_rules.md`, or the code's actual behavior (taxable-tier profit remains taxed even after vesting is reached)? **Reframed, not resolved, by `SF-LIQUIDITY-TAX-SEPARATION`** (`docs/knowledge/study_fund/SF_LIQUIDITY_TAX_SEPARATION.md`) — the binary six-year statement is no longer carried forward as accepted knowledge, but whether Israeli law imposes a seniority-based mechanism at actual withdrawal remains explicitly Unknown; see that object's §11 for what evidence would close it.
2. Whether it is intentional that the vesting/seniority exemption check is entirely absent from the PDF-verified calculation path. **Still open** — see `SF-LIQUIDITY-TAX-SEPARATION` and `SF-TAX-MODEL` §5 (Interaction & Edge Cases).
